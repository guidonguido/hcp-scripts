...
EXISTING_NS=$(aws route53 list-resource-record-sets \
    ...
    --query "ResourceRecordSets[?Name=='${BASE_DOMAIN}.' && Type=='NS'].ResourceRecords[0].Value" \
    --output text 2>/dev/null)

if [[ -n "${EXISTING_NS}" && "${EXISTING_NS}" != "None" ]]; then
    log_skip "NS delegation for ${BASE_DOMAIN} in ${PARENT_DOMAIN}"
else
    log_run "Adding NS delegation"
